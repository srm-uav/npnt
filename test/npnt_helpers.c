...
void reset_sha1() { SHA1_Init(&sha); }

void update_sha1(const char *data, uint16_t data_len) { SHA1_Update(&sha, data, data_len); }

void final_sha1(char *hash) { SHA1_Final((unsigned char *)hash, &sha); }
...
int8_t npnt_check_authenticity(npnt_s *handle, uint8_t *raw_data, uint16_t raw_data_len,
  ...
  if (ret == 0) {
    decSigLen = wc_RsaSSL_VerifyInline(signature, signature_len, &decSig, pRsaKey);
    if ((int)decSigLen < 0) {
      ret = (int)decSigLen;
    }
  }
  ...
  if (ret == 0 && decSigLen != raw_data_len) ret = -1;
  if (ret == 0 && XMEMCMP(raw_data, decSig, decSigLen) != 0) ret = -1;
