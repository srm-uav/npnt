 ...
 * @param[in] permart           permission json artefact in base64 format as
 * received from server
 * @param[in] permart_length    size of permission json artefact in base64
 * format as received from server
 * @param[in] signature         signature of permart in base64 format
 * @param[in] signature_length  length of the signature of permart in base64
 * format
 *
...
int8_t npnt_verify_permart(npnt_s *handle) {
  ...
  signature = (const uint8_t *)mxmlGetOpaque(mxmlFindElement(
      handle->parsed_permart, handle->parsed_permart, "SignatureValue", NULL, NULL, MXML_DESCEND));
  if (signature == NULL) {
    ret = NPNT_INV_SIGN;
    goto fail;
  }
  ...
  curr_ptr = 0;
  curr_length = 0;
  // Canonicalise Permission Artefact by converting Empty elements to start-end
  // tag pairs
  ...
  // handle->parsed_permart, "DigestValue", NULL, NULL, MXML_DESCEND)));

  // Check Digestion
  rcvd_digest_value = (const uint8_t *)mxmlGetOpaque(mxmlFindElement(
      handle->parsed_permart, handle->parsed_permart, "DigestValue", NULL, NULL, MXML_DESCEND));
...
int8_t npnt_alloc_and_get_fence_points(npnt_s *handle, float *vertlat, float *vertlon) {
  ...
  const char *lat_str;
  const char *lon_str;
  first_coordinate = mxmlGetFirstChild(mxmlFindElement(
      handle->parsed_permart, handle->parsed_permart, "Coordinates", NULL, NULL, MXML_DESCEND));
  ...
  while (current_coordinate) {
    ...
    // printf("\n%s %.20f %.20f\n", mxmlGetElement(current_coordinate),
    // vertlat[nverts], vertlon[nverts]);
    current_coordinate = mxmlGetNextSibling(current_coordinate);
    nverts++;
  }
  return nverts;
...
int8_t npnt_get_max_altitude(npnt_s *handle, float *altitude) {
  ...
  flightparams = mxmlFindElement(handle->parsed_permart, handle->parsed_permart, "FlightParameters",
...
int8_t npnt_populate_flight_params(npnt_s *handle) {
  ...
  ua_detail = mxmlFindElement(handle->parsed_permart, handle->parsed_permart, "UADetails", NULL,
                              NULL, MXML_DESCEND);
  if (!ua_detail) {
    return NPNT_INV_FPARAMS;
  }
  flight_params = mxmlFindElement(handle->parsed_permart, handle->parsed_permart,
                                  "FlightParameters", NULL, NULL, MXML_DESCEND);
  if (!flight_params) {
    return NPNT_INV_FPARAMS;
  }
  ...
  if (npnt_ist_date_time_to_unix_time(mxmlElementGetAttr(flight_params, "flightEndTime"),
                                      &handle->params.flightEndTime) < 0) {
    return NPNT_INV_FPARAMS;
  }
  if (npnt_ist_date_time_to_unix_time(mxmlElementGetAttr(flight_params, "flightStartTime"),
                                      &handle->params.flightStartTime) < 0) {
    return NPNT_INV_FPARAMS;
  }
  return 0;
}
